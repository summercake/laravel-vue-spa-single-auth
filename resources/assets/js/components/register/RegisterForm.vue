	...
	<div>
		<form class="form-horizontal" @submit.prevent="register">
			<div class="form-group" :class="{'has-error':errors.has('name')}">
				...
				<div class="col-md-7">
					<input v-model="name" v-validate="{rules: {required:true, min:4}}" id="name" type="text" class="form-control" name="name" required autofocus>
			...
			<div class="form-group" :class="{'has-error':errors.has('password_confirmation')}">
				...
				<div class="col-md-7">
					...
					<span class="help-block" v-show="errors.has('name')">{{errors.first('password_confirmation')}}</span>
				</div>
			</div>
